"""Command line entry point.
    ...
    python -m psm.cli seed --from evals/data/persona_seed.json
    python -m psm.cli inspect
    python -m psm.cli dictate "The Sarvam Kiwi service is down." --app com.tinyspeck.slackmacgap
...
from psm.config import from_env
from psm.domain.enums import ObservationSource
from psm.domain.models import Observation
...
def cmd_dictate(args: argparse.Namespace) -> int:
    ...
    from psm.engine.engine import Engine, summarise
...
def cmd_teach(args: argparse.Namespace) -> int:
    ...
    from psm.domain.enums import BindingScope, ObservationSource
    from psm.domain.models import Binding
...
def _quiet_broken_pipe() -> None:
    """`psm inspect | head` closes the pipe early. Without this, Python prints a
...
def main(argv: list[str] | None = None) -> int:
    _quiet_broken_pipe()
    parser = argparse.ArgumentParser(prog="psm")
    ...
    p_eval.add_argument(
        "--live", action="store_true", help="use a real model endpoint (needs credentials)"
    )
    p_eval.add_argument(
        "--replay", action="store_true", help="use recorded cassettes instead of a live model"
    )
