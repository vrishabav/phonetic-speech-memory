...
The base URL and the model name are configuration rather than constants, so
the same adapter drives any OpenAI-compatible endpoint - which is also what
lets `tests/test_live_path.py` point it at a stub server on localhost and
exercise the whole chain without a network. The key is read from the
environment, which `psm.config.load_dotenv_once` populates from `.env`. It is
never logged, never written to a cassette and never included in an eval
result.
"""
...
from psm.config import load_dotenv_once
    ...
    def complete(
        ...
        *,
        ...
        try:
            ...
            raise RuntimeError(f"{self.base_url} returned {exc.code}: {detail}") from exc
