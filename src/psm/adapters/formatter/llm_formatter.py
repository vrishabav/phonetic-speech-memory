...
class LLMFormatter:
    ...
    def __init__(
        ...
        **_: Any,
        ...
        self.calls = 0
        self.rejections = 0
        #: Calls where the model could not be reached at all. Counted rather
        #: than merely survived: degrading to the raw text is right in
        #: production and catastrophic in an evaluation, because a run that
        #: failed on every call looks exactly like a run that had nothing to do.
        self.failures = 0
        self.last_error: str = ""
    ...
    def format(
        ...
        *,
        ...
        try:
            ...
            # it. The raw text is a worse answer, not a broken one - but it is
            # recorded, because silence here once made an evaluation that
            # reached no model at all indistinguishable from a healthy one.
            self.failures += 1
