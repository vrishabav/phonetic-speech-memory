...
single-user personal-memory system, so a single instance is the right model;
...
from psm.api.schemas import adjudication_json, lexeme_json
from psm.api.ui import PAGE
from psm.config import Settings, from_env
from psm.domain.enums import ObservationSource
from psm.domain.models import Binding, BindingScope, Utterance
...
def _migrate(database_url: str) -> None:
    ...
    result = subprocess.run(
        ...
        cwd=ROOT,
        env={**os.environ, "PSM_DATABASE_URL": database_url},
        capture_output=True,
        text=True,
    )
...
def explorer():
    ...
    if not EXPLORER.exists():
        result = subprocess.run(
            [sys.executable, "-m", "psm.cli", "explore"],
            cwd=ROOT,
            capture_output=True,
            text=True,
        )
