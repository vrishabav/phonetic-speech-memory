...
class Settings:
    ...
    # --- infrastructure ---
    database_url: str = "sqlite:///./data/psm.db"
...
def load_dotenv_once() -> None:
    ...
    if _DOTENV_LOADED:
        return
    _DOTENV_LOADED = True
    path = Path(os.environ.get("PSM_DOTENV", ".env"))
    if not path.exists():
        return
...
def from_env(**overrides: Any) -> Settings:
    ...
    for field_name in (
        ...
        value = os.environ.get(f"PSM_{field_name.upper()}")
        if value:
            env[field_name] = value
    policies = os.environ.get("PSM_POLICIES")
    if policies:
